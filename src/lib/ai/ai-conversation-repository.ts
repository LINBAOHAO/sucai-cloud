...
/** In-memory store is only valid for local dev; serverless instances do not share it. */
...
function dbRequiredError(): Error {
  return new Error("Database unavailable. AI conversation storage requires a connected database.");
}
...
export async function listAdminConversations(
  ...
  noStore();
  if (!(await isDbAvailable())) {
    if (!(await shouldUseMemoryStore())) return [];
    const records = [...memoryConversations.values()];
    return records
      .map((record) => ({
        id: record.id,
