...
const emptyForm = (): Omit<AdminCustomer, "id" | "createdAt" | "updatedAt"> => ({
  companyName: "",
  contactName: "",
  email: "",
  whatsapp: "",
...
export default function AdminCustomersPage() {
  const { data: customers, refresh } = useAdminFetch<AdminCustomer[]>("/api/admin/customers");
    ...
    <>
      ...
      <div className="relative mb-4 max-w-md">
        ...
        <Input
          ...
          placeholder="搜索公司名、联系人、WhatsApp..."
            ...
            <tbody>
              {filtered.map((customer) => (
                <tr key={customer.id} className="border-b border-slate-50 hover:bg-slate-50/50">
                  <td className="px-4 py-3 font-medium text-slate-800">{customer.companyName}</td>
                  <td className="px-4 py-3 text-slate-600">{customer.contactName || "—"}</td>
                  <td className="px-4 py-3 text-slate-600">{customer.whatsapp || "—"}</td>
...
function CustomerForm({
  ...
  form: Omit<AdminCustomer, "id" | "createdAt" | "updatedAt">;
  setForm: React.Dispatch<
    React.SetStateAction<Omit<AdminCustomer, "id" | "createdAt" | "updatedAt">>
  >;
}) {
  return (
